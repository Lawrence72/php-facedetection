...
class FaceDetector {
    ...
    public function faceDetect($file) {
        ...
        $setWidth = floor($im_width / $ratio);
        $setHeight = floor($im_height / $ratio);

        if ($ratio != 0) {
            $this->reduced_canvas = imagecreatetruecolor($setWidth, $setHeight);
            ...
            if (!empty($this->face['w']) && $this->face['w'] > 0) {
                $this->face['x'] *= $ratio;
                $this->face['y'] *= $ratio;
                $this->face['w'] *= $ratio;
            }
        ...
        return (!empty($this->face['w']) && $this->face['w'] > 0);
    }
    ...
    protected function computeII($canvas, $image_width, $image_height) {
        ...
        for ($i = 1; $i < $ii_h - 1; $i++) {
            ...
            for ($j = 1; $j < $ii_w - 1; $j++) {
                ...
                $green = ($rgb >> 8) & 0xFF;
                $blue = $rgb & 0xFF;
                $grey = intval(0.2989 * $red + 0.587 * $green + 0.114 * $blue);  // this is what matlab uses
                $rowsum += $grey;
                $rowsum2 += $grey * $grey;
